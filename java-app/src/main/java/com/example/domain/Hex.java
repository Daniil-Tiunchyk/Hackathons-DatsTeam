...
public record Hex(int q, int r) {
    ...
    /**
     * Статический список, содержащий 6 векторов направления от центрального гекса к его соседям.
     * Используется для навигации по сетке. Порядок важен и соответствует стандартному обходу
    ...
    /**
     * Умножает координаты гекса (рассматриваемого как вектор) на скалярный множитель.
     * Это полезно для масштабирования векторов направления. Например, чтобы получить
     * гекс, находящийся на расстоянии 5 шагов в определенном направлении, можно взять
     * вектор этого направления и умножить его на 5.
     *
     * @param factor Целочисленный множитель.
     * @return Новый гекс с масштабированными координатами.
     */
    public Hex multiply(int factor) {
